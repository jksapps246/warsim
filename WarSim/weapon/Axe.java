...
public class Axe extends Weapon {
    ...
    private final String ANSI_BLUE = "\u001B[34m";//blue color

    public Axe() {//constructor
        super();
        setStats();
        setImage();
    }
    ...
    public int strike(int attackType, int strength, 
        ...
        if(special){
            ...
            if(attackType == 1) { // basic attack
                //hit += strength/25;
                hit += randNum.nextInt(mobility)+1 + luck;
            }
            else { // heavy attack
                hit += randNum.nextInt(mobility)+1;
            }
        ...
        if(roll <= hit) { // hit
            damage = (int)Math.round(DAMAGE_AMOUNT + (DAMAGE_AMOUNT * (strength / 100)));//increase by strength as a percentage
